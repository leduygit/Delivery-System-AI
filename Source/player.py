# player.py
import pygame
from config import GRID_SIZE, PLAYER_IMAGE_SIZE, FRAME_DELAY
    ...
    def load_images(self):
        for player_num in range(1, 5):
            self.player_images[f'player{player_num - 1}'] = {
                'left': [pygame.transform.scale(pygame.image.load(f'Assets/Images/Players/player{player_num}-left-{i}.png'), PLAYER_IMAGE_SIZE) for i in range(12)],
                'right': [pygame.transform.scale(pygame.image.load(f'Assets/Images/Players/player{player_num}-right-{i}.png'), PLAYER_IMAGE_SIZE) for i in range(12)],
                'idle': pygame.transform.scale(pygame.image.load(f'Assets/Images/Players/player{player_num}-idle.png'), PLAYER_IMAGE_SIZE)
            }
