            ...
            self.player_images[f"agent_{player_num}"] = {
                "left": [
                    pygame.transform.scale(
                        pygame.image.load(
                            f"{IMAGE_FOLDER}/Players/player {player_num}/player{player_num}-left-{i}.png"
                        ),
                ...
                "right": [
                    pygame.transform.scale(
                        pygame.image.load(
                            f"{IMAGE_FOLDER}/Players/player {player_num}/player{player_num}-right-{i}.png"
                        ),
                        config.PLAYER_IMAGE_SIZE,
                    )
                    for i in range(12)
                ...
                "idle": pygame.transform.scale(
                    pygame.image.load(
                        f"{IMAGE_FOLDER}/Players/player {player_num}/player{player_num}-idle.png"
                    ),
                    config.PLAYER_IMAGE_SIZE,
                ),
            }
    ...
    def draw(self, screen, players, frame_count):
        for player_name, player_data in players.items():
            ...
            player_image = self.get_player_image(
                player_name, direction, frame_count // FRAME_DELAY, is_idle
            )
            screen.blit(
                player_image,
                (
                    current_position[1] * config.GRID_SIZE
                    + self.offset[0]
                    + config.GRID_SIZE * 0.15,
                    current_position[0] * config.GRID_SIZE + self.offset[1] - 1,
                ),
            )
            ...
            pygame.draw.rect(
                ...
                pygame.Color("black"),
                (
                    goal_position[1] * config.GRID_SIZE + self.offset[0],
                    goal_position[0] * config.GRID_SIZE + self.offset[1],
                    config.GRID_SIZE,
                    config.GRID_SIZE,
                ),
                2,
